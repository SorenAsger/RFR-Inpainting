...
class MobileNetFeatureExtractor(nn.Module):
    def __init__(self):
        ...
        self.enc_1 = nn.Sequential(*mnet.features[:3])
        self.enc_2 = nn.Sequential(*mnet.features[3:6])
        self.enc_3 = nn.Sequential(*mnet.features[6:9])
        self.enc_4 = nn.Sequential(*mnet.features[9:])
        # fix the encoder
        for i in range(4):
            for param in getattr(self, 'enc_{:d}'.format(i + 1)).parameters():
                param.requires_grad = False
        ...
        if finetune:
            for name, module in self.named_modules():
                if isinstance(module, nn.BatchNorm2d):
                    module.eval()
#effnet = models.vgg16(pretrained=True)
#print(len(effnet.features))
#print(effnet.features)
# print("===" * 100)
# print("===" * 100)
